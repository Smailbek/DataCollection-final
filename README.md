...
The pipeline is orchestrated with Apache Airflow, uses Apache Kafka for streaming, and SQLite as the final storage and analytics layer.
...
The API provides real-time aircraft state vectors, including:
- ICAO24 identifier
- Callsign
- Origin country
- Latitude / Longitude
...
- Fetches aircraft state data from the OpenSky API
- Publishes raw JSON messages to the Kafka topic opensky_raw_states

2. opensky_kafka_to_sqlite_hourly
Schedule: hourly
- Consumes messages from Kafka
- Cleans and normalizes incoming data
- Stores processed records into the SQLite table events

3. opensky_sqlite_daily_summary
Schedule: daily
- Aggregates data from the events table
- Computes daily statistics:
    - Total number of events
    - Number of unique countries
    - Average and maximum velocity
    - Average and maximum altitude
- Stores results in the daily_summary table


Database Schema

Table: events
Stores normalized aircraft event data.
Example fields:
- icao24
- callsign
- origin_country
...
- 
Trigger DAGs in the following order:
1. opensky_api_to_kafka
2. opensky_kafka_to_sqlite_hourly
3. opensky_sqlite_daily_summary
...
Result
- Real-time aircraft data is successfully ingested from the OpenSky API
- Data is streamed through Kafka
- Processed and stored using Airflow
- Daily analytics are computed and persisted in SQLite

Conclusion
This project demonstrates a fully functional data engineering pipeline covering:
- API ingestion
